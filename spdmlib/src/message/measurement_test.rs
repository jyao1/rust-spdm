...
fn test_measurement_response() {
    create_spdm_context!(context);
    let context = &mut context;
    // Validate responder measurement record length is beyond USER_MAX_SPDM_MSG_SIZE.
    let u8_slice = &mut [0u8; USER_MAX_SPDM_MSG_SIZE + 200];
    u8_slice[3].set_bits(4..=5, 0b10);
    u8_slice[3].set_bits(0..=3, 1);
    u8_slice[4] = 0xfe;
    LittleEndian::write_u24(&mut u8_slice[5..8], USER_MAX_SPDM_MSG_SIZE as u32);
    LittleEndian::write_u16(
        &mut u8_slice[(40 + USER_MAX_SPDM_MSG_SIZE)..(42 + USER_MAX_SPDM_MSG_SIZE)],
        1024,
    );
