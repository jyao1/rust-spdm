...
pub fn fuzz_total_requesters() {
    ...
    let result = requester.start_session(false, 0, SpdmMeasurementSummaryHashType::SpdmMeasurementSummaryHashTypeNone);
    if let Ok(session_id) = result {
        ...
        log::info!("equest_direction.encryption_key {:0x?}\n", request_direction.encryption_key.as_ref());
        log::info!("equest_direction.salt {:0x?}\n", request_direction.salt.as_ref());
        log::info!("esponse_direction.encryption_key {:0x?}\n", response_direction.encryption_key.as_ref());
        log::info!("esponse_direction.salt {:0x?}\n", response_direction.salt.as_ref());
